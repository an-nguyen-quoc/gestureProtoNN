...
Featurizer::Featurizer(
    int _bucketWidth, 
    FIFOCircularQ<float, 10> *_ngon_1, 
    FIFOCircularQ<float, 10> *_ngon_2,
    FIFOCircularQ<float, 10> *_ngon_3,
    FIFOCircularQ<float, 10> *_ngon_4,
    FIFOCircularQ<float, 10> *_ngon_5,
    FIFOCircularQ<float, 10> *_x,
    FIFOCircularQ<float, 10> *_y,
    FIFOCircularQ<float, 10> *_z
    ){
    if(_bucketWidth != 10) 
        exit(-1);
    this->ngon_1 = _ngon_1;
...
int Featurizer::getBucket(
    FIFOCircularQ<float, 10>*sensorValue1D,
    ...
    for(int i =0; i < 10; i++){
        ...
        if(val < 0){
            ...
            bucketDistribution[bucketIndex+9]+=1;
        } else {
            int index=val/10;
